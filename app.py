...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:2007@localhost/fraud_db'
...
# ======== PATHS ========
MODEL_PATH = r"C:/Users/HP/Desktop/py/machine_learning/Dossier/rf_pipeline2.pkl"
CSV_PATH = r"C:/Users/HP/Desktop/py/machine_learning/Dossier/fraudTrain.csv"
...
def dashboard():
    ...
    if not user_transactions:
        flash("You haven't made any predictions yet.", "info")
    ...
    colors = ["#35727B", "#6C1995"]
    pie_fig = px.pie(values=fraud_counts.values, names=labels, title="My Predictions: Fraud vs Normal",
                     color_discrete_sequence=colors)
    pie_fig.update_layout(paper_bgcolor='#0a1929', plot_bgcolor='#0a1929',
    ...
    if 'gender' in df.columns and df['gender'].notna().any():
        ...
        gender_fig.update_layout(paper_bgcolor='#0a1929', plot_bgcolor='#0a1929',
    ...
    time_fig = px.line(time_counts_sorted, x='datetime', y='cumulative_count', color='label',
                       title="Number of My Predictions Over Time",
                       labels={'datetime': 'Date and Time', 'cumulative_count': 'Number of Predictions', 'label': 'Type'},
                       color_discrete_map={'Normal': "#35727B", 'Fraude': "#6C1995"},
                       markers=True)
    time_fig.update_traces(line=dict(width=3), marker=dict(size=10))
    time_fig.update_xaxes(title_text="Date and Time", tickformat="%Y-%m-%d\n%H:%M",
                          gridcolor='rgba(148, 163, 184, 0.1)')
    time_fig.update_yaxes(title_text="Number of Predictions", dtick=1,
                          gridcolor='rgba(148, 163, 184, 0.1)')
    time_fig.update_layout(hovermode='closest', paper_bgcolor='#0a1929', plot_bgcolor='#0a1929',
                           font=dict(color='white'), title_font=dict(color='#94a3b8', size=16))
    ...
    if request.method == 'POST':
        ...
        if analysis_type == 'univarie' and len(cols) == 1:
            ...
            if graph_type == 'histogram':
                ...
                fig = px.bar(vc, x=col, y='count', title=f'Barres de {col}')
        ...
        if fig is not None:
            fig.update_layout(paper_bgcolor='#0a1929', plot_bgcolor='#0a1929',
                              font=dict(color='white'), title_font=dict(color='#94a3b8', size=16))
...
@login_required
def user_analysis():
    user_tx = Transaction.query.filter_by(user_id=current_user.id)\
                               .order_by(Transaction.created_at).all()

    user_cols = ["category", "amt", "gender", "city",
                 "state", "job", "city_pop",
                 "merch_lat", "merch_long"]

    if not user_tx:
        return render_template("user_analysis.html",
                               pie_html=None, box_html=None, custom_html=None,
                               user_cols=user_cols, username=current_user.username)
    ...
    pie_fig = px.pie(values=fraud_count.values, names=labels,
                     title="Répartition Normal / Fraude",
                     color_discrete_sequence=["#35727B", "#6C1995"])
    pie_fig.update_layout(paper_bgcolor="#0a1929", plot_bgcolor="#0a1929", font=dict(color="white"))
    pie_html = pie_fig.to_html(full_html=False)

    # Box
    box_fig = px.box(df, x="prediction", y="amt", color="prediction",
                     title="Distribution des montants",
                     color_discrete_map={0: "#35727B", 1: "#6C1995"})
    box_fig.update_layout(paper_bgcolor="#0a1929", plot_bgcolor="#0a1929", font=dict(color="white"))
    box_html = box_fig.to_html(full_html=False)

    # Custom
    custom_html = None
    if request.method == "POST":
        analysis_type = request.form.get("user_analysis_type")
        col1 = request.form.get("u_col1")
        col2 = request.form.get("u_col2")
        col3 = request.form.get("u_col3")
        graph_type = request.form.get("u_graph_type")

        cols = [c for c in [col1, col2, col3] if c]
        fig = None
        ...
        if analysis_type == "univarie" and len(cols) == 1:
            ...
            x, y = cols
            if graph_type == "scatter":
                fig = px.scatter(df, x=x, y=y, color="prediction")
            elif graph_type == "box":
            ...
            fig = px.scatter_3d(df, x=cols[0], y=cols[1], z=cols[2], color="prediction")

        if fig is not None:
            fig.update_layout(paper_bgcolor="#0a1929", plot_bgcolor="#0a1929", font=dict(color="white"))
            custom_html = fig.to_html(full_html=False)

    return render_template("user_analysis.html",
                           pie_html=pie_html, box_html=box_html, custom_html=custom_html,
                           user_cols=user_cols, username=current_user.username)
