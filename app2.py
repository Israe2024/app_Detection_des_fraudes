...
if not os.path.exists(CSV_PATH):
    ...
    gdown.download(
        f"https://drive.google.com/uc?id={DATA_ID}",
        CSV_PATH,
        quiet=False
    )
...
@login_required
def home():
    history = Transaction.query.filter_by(user_id=current_user.id).order_by(Transaction.created_at.desc()).limit(10).all()
